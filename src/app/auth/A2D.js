...
| pc2bytes13[(right >>> 4) & 0xf];
        ...
        this.encrypt = function (s) {
            return stringToHex(des(this.key, s, 1, 0));
        }

        this.decrypt = function (s) {
            return des(this.key, HexTostring(s), 0, 0);
        }
        ...
        this.get = function (name) {
            ...
            while (i < clen) {
                ...
                if (doc.cookie.substring(i, j) == arg) {
                    return A2D.DES.decrypt(getCookieVal(j));
                }
    ...
    if (!window.DESKey)
        window.DESKey = "12345678";
    A2D.DES = desEncoder(window.DESKey);
    A2D.Cookie = securityCookie();
